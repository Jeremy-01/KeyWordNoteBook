                        参考资料
## 文档说明
    本文件为开发过程中的外部资料摘录与知识备忘，不作为正式需求、设计说明或协作规范。
...
## 本次会话更新（2026-04-29）
###
    （固定模板：后续会话请按“功能 / 性能 / 安全 / Git”四小节续写）

### 功能
    1. 修复基础运行问题：调整 main.py 样式与缩进，完成依赖检查，程序可正常启动。
    2. 新增 CSV 导入：在 UI 中加入浏览器密码 CSV 导入流程，并补充导入结果提示与使用说明。
    3. 准备测试数据：通过现有加密流程写入测试条目（含弱口令与重复口令样本），用于功能与统计验证。

### 性能
    1. UI 侧优化：实现重复密码分段缓存、筛选防抖、延迟落盘触发。
    2. Core 侧优化：实现延迟同步与批量写入接口，降低高频 I/O。
    3. 重复检测优化：由全量解密扫描升级为 Core 哈希索引缓存，并在新增/修改/删除时增量维护。
    4. 验证结果：静态检查与行为验证通过，重复项统计在增删改后符合预期变化。

### 安全
    1. 敏感文件策略：my_key.json 继续仅本地使用，不纳入版本库。
    2. 演示文件建议：对外演示使用 my_key.example.json 模板，避免真实敏感信息泄露。

### Git
    1. 更新 .gitignore：补充 my_key.json、.venv、.claude、缓存与构建产物等忽略规则。
    2. 同步远端仓库：已推送 GitHub，commit 为 7037728（chore: update gitignore for local env and artifacts）。
...
## 本次会话更新（2026-04-29，文档与发布）
###
    （固定模板：后续会话请按“功能 / 性能 / 安全 / Git”四小节续写）

### 功能
    1. 增加剪贴板到期“强制清空模式”配置开关：`KEYWORD_NOTEBOOK_CLIPBOARD_FORCE_CLEAR`。
    2. 状态栏文案区分三种结果：已自动清空 / 已强制清空 / 未清空（用户已改写）。
    3. 更新 README：补充剪贴板安全策略、环境变量说明与版本补丁记录。

### 性能
    1. 剪贴板清理采用实例级单次定时器复用（stop/start），减少多次复制时的时序干扰。
    2. 新增测试聚焦行为正确性，避免后续改动造成回归（间接降低排障成本）。

### 安全
    1. 默认模式保持“用户改写不强删”策略，避免误删用户后续剪贴板内容。
    2. 可选强制清空模式可用于高安全场景，降低敏感信息残留风险。
    3. 补齐单元测试：未改写清空、改写不清空、强制清空、连续复制计时重置。

### Git
    1. 本次计划提交文档与相关改动并推送远端，提交信息将覆盖剪贴板安全补丁与文档更新。

## 本次会话更新（2026-04-29，v2会话派生强化）
###
    （固定模板：后续会话请按“功能 / 性能 / 安全 / Git”四小节续写）

### 功能
    1. v2规则调整：会话派生参数升级为强参数，与认证参数保持一致。
    2. 新增兼容路径：支持历史v2轻参数数据读取，避免升级后旧库无法解密。
    3. 自动迁移落盘：兼容读取成功后，自动将旧密文重写为强参数密文。

### 性能
    1. 常规会话仍复用派生缓存，避免每次加解密重复派生。
    2. 兼容迁移仅在检测到历史v2轻参数时触发，迁移完成后恢复常规路径。

### 安全
    1. 会话派生从轻参数升级为强参数，提升离线暴力破解成本。
    2. 历史轻参数仅用于一次性兼容读取，不再用于后续新写入。
    3. 迁移覆盖条目密文、常用密码密文与HMAC密钥密文，保持完整性校验链路一致。

### Git
    1. 更新 README 与参考资料技术说明，反映v2会话派生强化与兼容迁移策略。
    2. 推送时保持仓库边界策略，不恢复同步 `.gitignore`、`.idea/`、`tests/` 等本地路径。
